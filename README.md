...
```
提问 → 大模型生成回答 → 画像提取器判断「有没有透露新信息」→ 有则更新画像存 SQLite → 下次提问时注入
```

- **写入**：每轮回答完，单独调一次大模型判断用户是否透露了值得长期记住的信息（姓名、职位、部门、偏好等稳定信息），有就合并进画像存到 users 表；临时信息（「今天想请假」）不记。这一步在后台线程做，不阻塞回答
- **注入**：下次提问时把画像拼进系统提示词，全新会话也能记住用户是谁
